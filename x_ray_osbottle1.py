...
import random
from randomflip import rotate_image_with_background
...
# 主函数部分
# 创建保存结果的文件夹
if not os.path.exists('small_data_bottle_f'):
    os.makedirs('small_data_bottle_f')

# 遍历small_data文件夹中的所有图片
for bg_file in os.listdir('samples/small_data'):
    try:
        if bg_file.endswith(('.jpg', '.png', '.jpeg')):
            # 读取obj_img
            obj_img = cv2.imread('samples/bottle1.jpg')
            obj_img = rotate_image_with_background(obj_img)
            ...
            aap, bbp = find_best_rec(1.9, bg_img, obj_img) # 调整梯度大小
            ...
            # 使用plt保存mix_img图片
            output_filename = os.path.join('small_data_bottle_f', f'result_{bg_file}')
