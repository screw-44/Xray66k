...
def find_best_rectangle(bg_img: np.ndarray, obj_mask: np.ndarray) -> Tuple[int, int, int, int]:
    obj_coords = np.column_stack(np.where(obj_mask > 0))
    x_min, y_min = np.min(obj_coords, axis=0)
    x_max, y_max = np.max(obj_coords, axis=0)

    obj_height, obj_width = y_max - y_min + 1, x_max - x_min + 1

    best_variation = float('inf')
    best_rect = (0, 0, 0, 0)

    # Slide a window over the background image
    bg_h, bg_w = bg_img.shape[:2]
    for y in range(bg_h - obj_height):
        for x in range(bg_w - obj_width):
            # Extract the candidate rectangle
            candidate = bg_img[y:y + obj_height, x:x + obj_width]
            # Compute the variation within the candidate rectangle
            variation = np.std(candidate)  # You can also use other metrics like np.mean or np.var

            if variation < best_variation:
                best_variation = variation
                best_rect = (x, y, obj_width, obj_height)
...
blue_threshold = 20
mix_img[y_offset+aa:y_offset+obj_img_h+aa, x_offset+bb:x_offset+obj_img_w+bb, 0] \
    = np.minimum(bg_img[y_offset+aa:y_offset+obj_img_h+aa, x_offset+bb:x_offset+obj_img_w+bb, 0], new_obj_img[y_offset+aa:y_offset+obj_img_h+aa, x_offset+bb:x_offset+obj_img_w+bb, 0])
# 确保只在掩膜区域进行操作
for y, x in zip(mask_y, mask_x):
    if mix_img[y_offset+y+aa, x_offset+x+bb, 0] < blue_threshold:  # 检查是否为蓝色
        mix_img[y_offset+y+aa, x_offset+x+bb, 0] = bg_img[y_offset+y+aa, x_offset+x+bb, 0]  # 用背景替换
    else:
        mix_img[y_offset+y+aa, x_offset+x+bb, 0] = np.minimum(bg_img[y_offset+y+aa, x_offset+x+bb, 0], new_obj_img[y_offset+y+aa, x_offset+x+bb, 0])
