...
# 主函数部分
# 创建保存结果的文件夹
if not os.path.exists('small_data_fire'):
    os.makedirs('small_data_fire')

# 读取obj_img
obj_img = cv2.imread('samples/fire1.jpg')
hsv_obj = cv2.cvtColor(obj_img, cv2.COLOR_BGR2HSV)
# 定义黑色背景的颜色范围
lower_white = np.array([0, 0, 250])  # 白色的HSV范围
...
obj_img = np.where(result == 0, background, result)  # 将黑色背景替换为白色
...
for bg_file in os.listdir('samples/small_data'):
    try:
        ...
        # 使用plt保存mix_img图片
        output_filename = os.path.join('small_data_fire', f'result_{bg_file}')
