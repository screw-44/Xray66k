...
def find_best_rec(target_gradient: float, bg_hsv: np.ndarray, obj_hsv: np.ndarray) -> Tuple[int, int]:
    ...
    for y in range(bg_h - obj_height + 1):
        for x in range(bg_w - obj_width + 1):
            # 提取重叠区域的Hue通道
            candidate = bg_hsv[y:y + obj_height, x:x + obj_width, 0]  # Hue通道
            ...
            if overlap_hue.size > 0:
                ...
                # 检查是否在目标梯度值的范围内
                if target_gradient - 0.01 <= hue_variation <= target_gradient + 0.01:
                    # 计算偏移量
                    aa = y - y_center
                    bb = x - x_center
                    return aa, bb  # 找到符合条件的矩形，返回偏移量
...
aap, bbp = find_best_rec(3.52,bg_img,obj_img)
print(aap,bbp)
aa=aap
bb=bbp
# aa=150
