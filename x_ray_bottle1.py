...
def find_best_rec(target_gradient: float, bg_hsv: np.ndarray, obj_hsv: np.ndarray) -> Tuple[int, int]:
    ...
    if valid_positions:
        ...
        # 随机选取一组aa,bb，满足绝对值均小于100
        valid_positions = [pos for pos in valid_positions if abs(pos[0]) < 100 and abs(pos[1]) < 100]
...
# 使用plt保存mix_img图片
output_filename = os.path.join('output', 'blended_image.jpg')
